...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for current_input in np.swapaxes(x, 0, 1):
        current_h, current_cache = rnn_step_forward(current_input, previous_h, Wx, Wh, b)
...
def rnn_backward(dh, cache):
    ...
    for layer_idx in range(T - 1, -1, -1):
        ...
        (curr_dx, 
         ...
         curr_dWx, 
         curr_dWh, 
         curr_db) = rnn_step_backward(dh[:, layer_idx, :] + dh0, layer_cache)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        ...
        next_h, next_c, current_cache = lstm_step_forward(x[:, t, :], 
                                                          prev_h, 
                                                          prev_c, 
                                                          Wx, Wh, b)
        cache.append(current_cache)
...
def lstm_backward(dh, cache):
    ...
    curr_dh = np.zeros((N, H))
    curr_dc = np.zeros((N, H))
    ...
    for t in range(T - 1, -1, -1):
        (curr_dx, 
         ...
         curr_Wx, 
         curr_Wh, 
         curr_db) = lstm_step_backward(dh[:, t, :] + curr_dh, 
                                       curr_dc, 
                                       cache[t])
